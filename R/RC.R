#' RCBD Analysis (Randomized Complete Block Design)
#'
#' Perform ANOVA, assumption checks, and mean separation for RCBD experiments.
#'
...
#' @param block Column name for the blocking factor.
...
#' )
#' }
RC <- function(data, trait, treatment, block,
               var_test = "bartlett",
               norm_test = "shapiro",
               mean_method = "lsd",
               check_normality = TRUE,
  ...
  for(tr in trait){
    cat("\n=====================================\n")
    cat("RCBD ANALYSIS — Trait:", tr, "\n")

    formula <- as.formula(paste(tr, "~", treatment, "+", block))
