#' CRD Analysis (Completely Randomized Design)
#'
#' Perform ANOVA, assumption checks, and mean separation for CRD experiments.
#'
...
#' )
#' }
CR <- function(data, trait, treatment,
               var_test = "bartlett",
               norm_test = "shapiro",
               mean_method = "lsd",
               check_normality = TRUE,
  ...
  for(tr in trait){
    cat("\n=====================================\n")
    cat("CRD ANALYSIS — Trait:", tr, "\n")

    formula <- as.formula(paste(tr, "~", treatment))
